...
(function () {
    ...
    TextMeasurer = function () {
        this.measureText = function (options) {
            var fontSize, fontWeight, fontFamily, font, boundingRect, canvas, context, imageData, result;
            fontSize = options.fontSize || getComputedStyle(options.element, 'font-size') || '100px';
            fontWeight = options.fontWeight || getComputedStyle(options.element, 'font-weight') || 'normal';
            fontFamily = options.fontFamily || getComputedStyle(options.element, 'font-family') || 'Arial';
            font = fontWeight + ' ' + fontSize + ' ' + fontFamily;

            boundingRect = this.measureBoundingRect(options.text, font);
            ...
            result = this.measure({ width: canvas.width, height: canvas.height, imageData: imageData, options: options });
        ...
        this.measureBoundingRect = function (text, font) {
            var temporaryContainer, temporaryElement, bBox;
            ...
            bBox = temporaryElement.getBoundingClientRect();
        ...
        this.measure = function (parameters) {
            var iterator, foundTopText, foundBottomText, topTextY, bottomTextY, allPixelsInThisRowAreWhite;
            ...
            iterator = new Iterator(parameters.width, parameters.height);
            foundTopText = false;
            foundBottomText = false;
            topTextY = null;
            bottomTextY = null;
